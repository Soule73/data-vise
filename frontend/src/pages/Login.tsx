...
export default function Login() {
  const { register, handleSubmit, formState: { errors }, onSubmit, globalError, loading } = useLoginForm();
  ...
  return (
    <AuthLayout
      ...
      bottomText={<>
        ...
        <a href="/register" className="font-semibold text-indigo-600 hover:text-indigo-500">Créer un compte</a>
      </>}
    >
      {globalError && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Button type="submit" color="indigo" size="md" variant="solid" loading={loading} disabled={loading}>Se connecter</Button>
      </form>
    </AuthLayout>
  );
}
