...
import { login as loginService, register as registerService } from "@/data/services/auth";
...
export function useUpdateUserMutation({
    ...
    return useMutation({
        mutationFn: async ({ id, payload }: { id: string; payload: any }) => updateUser(id, payload),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["users"] });
            onSuccess?.();
        },
