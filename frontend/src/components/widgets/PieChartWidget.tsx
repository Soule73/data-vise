...
export default function PieChartWidget({ data, config }: { data: any[]; config: any }) {
  ...
  const labels = data.map((row: any) => row[config.nameField]);
  const values = data.map((row: any) => row[config.valueField]);
