...
export default function DashboardGrid({
  ...
  } = useDashboardGrid({ layout, editMode, hasUnsavedChanges, onSwapLayout });
        ...
        if (idx === slots.length - 1 ) {
          ...
          return (
            <div
              ...
              style={{ gridColumn: `span ${isMobile ? 1 : 6} / span ${isMobile ? 1 : 6}` }}
              onClick={() => editMode && onSwapLayout && onSwapLayout(layout)}
            >
              <svg width="32" height="32" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
        ...
        return (
          <div
            ...
            className={`relative min-h-[160px] 
              ...
              hoveredIdx === idx && draggedIdx !== null ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 bg-gray-50 dark:bg-gray-800 dark:border-gray-800'
            }`}
            draggable={editMode && !!widget}
            onDragStart={editMode ? () => handleDragStart(idx) : undefined}
            onDragOver={editMode ? e => { e.preventDefault(); handleDragOver(idx); } : undefined}
            onDrop={editMode ? () => handleDrop(idx) : undefined}
            onDragEnd={editMode ? handleDragEnd : undefined}
            style={{
              gridColumn: `span ${item?.w ?? (isMobile ? 1 : 6)} / span ${item?.w ?? (isMobile ? 1 : 6)}`,
              gridRowEnd: `span ${Math.ceil((item?.h ?? 8) / 8)}`,
            }}
          >
            {widget ? (
              <div className="h-full w-full flex flex-col relative group">
                <DashboardWidgetPreview widget={widget} sources={sources} />
                {!isMobile && editMode && (
                  <div
                  ...
                  >
                    <svg width="16" height="16" viewBox="0 0 16 16">
                      <path d="M2 14h12M6 10l6 4" stroke="#555" strokeWidth="1.5" fill="none"/>
                    </svg>
                  </div>
                )}
              </div>
