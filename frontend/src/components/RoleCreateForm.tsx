...
export default function RoleCreateForm({ permissions, onSuccess }: RoleCreateFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim()) {
      useNotificationStore.getState().showNotification({ open: true, type: 'error', title: 'Nom requis', description: 'Le nom du rôle est obligatoire.' });
      return;
    }
    try {
      ...
      useNotificationStore.getState().showNotification({ open: true, type: 'success', title: 'Rôle créé', description: 'Le nouveau rôle a été ajouté.' });
      onSuccess();
    } catch (e) {
      useNotificationStore.getState().showNotification({ open: true, type: 'error', title: 'Erreur', description: 'Erreur lors de la création du rôle.' });
    }
  };
