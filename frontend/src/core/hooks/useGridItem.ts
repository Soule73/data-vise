...
export function useGridItem({
    ...
    }, [editMode, idx, handleDragStart, handleDragOver, handleDrop, handleDragEnd]);

    // Style props centralisés
    const styleProps = useMemo(() => {
        let className = "relative bg-white dark:bg-gray-800 rounded-lg shadow p-2 transition-all duration-200 overflow-hidden border-dashed border-2 border-spacing-4 ";
        if (editMode) {
    ...
    const source = sources?.find((s: any) => String(s._id) === String(widget?.dataSourceId));
    ...
    const widgetDef = widget ? WIDGETS[widget.type as keyof typeof WIDGETS] : null;
